...
How do we calculate the denotation of phrases like those in \REF{ex:15.10}? Although they cannot be defined as a simple intersection, the phrases are still compositional; knowing what each word means allows us to predict the meanings of the phrases. The trick is that with adjectives like these, as with propositional attitude verbs, we need to combine senses rather than denotations. In other words, these adjectives are intensional: they combine with the senses of the nouns they modify. Informal definitions of \textit{former} and \textit{alleged} are suggested in \REF{ex:15.11}:\largerpage
...
\ea \label{ex:15.37}
\begin{tabbing}\relax
[$\text{λ}x. \text{LOVE}(x,c) \wedge \text{HATE}(x,b)](a)$  \= $= \text{LOVE}(a,c)$  \=  ‘Caesar loves Brutus’ \kill
[$\text{λ}y. \text{LOVE}(c,y)](b)$ \> $= \text{LOVE}(c,b)$ \> ‘Caesar loves Brutus’ \\
{[$\text{λ}x. \text{LOVE}(x,b)](c)$ } \> = $\text{LOVE}(c,b)$ \> ‘Caesar loves Brutus’ \\
{[$\text{λ}x. \text{LOVE}(x,c) \wedge \text{HATE}(x,b)](a)$} \> $= \text{LOVE}(a,c) \wedge \text{HATE}(a,b)$
\end{tabbing}
...
\ea \label{ex:15.38} 
\begin{tabbing}\relax
[$\text{λ}x. \text{LOVE}(x,c)\wedge \text{HATE}(x,b)](n)$  \= $=$ \=  1 iff n loves Caesar and hates Brutus \kill 
[$\text{λ}y. \text{LOVE}(c,y)](n)$  \> $=$ \>  1 iff Caesar loves n \\
                    \>   \> 0 otherwise \\
{[$\text{λ}x. \text{LOVE}(x,b)](n)$}  \> = \>  1 iff n loves Brutus \\
                    \>   \> 0 otherwise \\
{[$\text{λ}x. \text{LOVE}(x,c)\wedge \text{HATE}(x,b)](n)$}  \> $=$ \>  1 iff n loves Caesar and hates Brutus \\
                    \>   \> 0 otherwise \\
\end{tabbing}
\z 

This means that the semantic value of an intransitive predicate like \textit{snore} can be represented as a function which takes a single argument: [$\text{λ}x$.~$\text{SNORE}(x)$]. The semantic value of the sentence \textit{Henry snores} can be derived by applying this function to the semantic value of the subject NP, as shown in \REF{ex:15.39}:


\ea \label{ex:15.39}
\begin{tabbing}\relax
[$\text{λ}x$. $\text{SNORE}(x)](h)$  \= = \=  1 iff Henry snores \kill
[$\text{λ}x$. $\text{SNORE}(x)](h)$  \> = \>  $\text{SNORE}(h)$\\
                   \> = \> 1 iff Henry snores\\
                   \>   \> 0 otherwise
\end{tabbing}
\z
...
The semantic value of a transitive predicate like \textit{love} can be represented as a function which takes two arguments: [$\text{λ}y$.~[$\text{λ}x$.~$\text{LOVE}(x,y)$]]. In calculating the truth conditions for a sentence like \textit{Caesar loves Brutus}, the function named by the verb is applied first to the semantic value of the object NP, as shown in (\ref{ex:15.40}a), to derive the semantic value of the VP. The function named by the VP is then applied to the semantic value of the subject NP, as shown in (\ref{ex:15.40}b), to derive the semantic value of the sentence as a whole.


\ea \label{ex:15.40}
\ea  {[$\text{λ}y$. [$\text{λ}x$. $\text{LOVE}(x,y)]](b) = [\text{λ}x$. $\text{LOVE}(x,b)]$}\\
\hfill  ‘is someone who loves Brutus’
\ex  {[$\text{λ}x$. $\text{LOVE}(x,b)](c) =  \text{LOVE}(c,b)$} \hfill ‘Caesar loves Brutus’
\z \z
