...
In the same way, the operator $\wedge$ ‘and’ can be defined by the truth table in \REF{ex:4.7}. This table says that $p \wedge q$ (which is also sometimes written \textit{p}\,\&\,\textit{q}) is true just in case both \textit{p} and \textit{q} are true, and false in all other situations.
...
The \textsc{material} \textsc{implication} operator ($\rightarrow$) is defined by the truth table in \REF{ex:4.10}. (The formula $p \rightarrow q$ can be read as \textit{if p (then) q}, \textit{p only if q}, or \textit{q if p}.) The truth table says that $p \rightarrow q$ is defined to be false just in case \textit{p} is true but \textit{q} is false; it is true in all other situations.
...
For convenience we will introduce one additional operator here, which is referred to as the \textsc{biconditional} operator ($\leftrightarrow$). The formula $p\leftrightarrow q$ (read as ‘p if and only if q’) is a short-hand or abbreviation for: ($p\rightarrow q$) $\wedge$ ($q\rightarrow p$). The biconditional operator is defined by the truth table in \REF{ex:4.11}:
...
\begin{xlist} 
\setcounter{xnumii}{1}
\exbox{\begin{tabular}[t]{>{\sffamily}c>{\sffamily}c>{\sffamily}c}
\lsptoprule
\tablehead{
 p & ¬p & p$\vee$(¬p)\\\midrule
}
...
\largerpage
The final step in the proof is to calculate the possible truth values of the proposition $p\vee (\neg p)$, using the truth table in \REF{ex:4.8} which defines the \textit{$\vee$} operator. The result is shown in (\ref{ex:4.12}c). 
...
\tablehead{ 1 & 2 & 3 & 4 & 5 & 6 \\
 p & q & p$\vee$q & ¬p & (p$\vee$q)$\wedge$¬p & ((p$\vee$q)$\wedge$¬p) $\rightarrow$ q\\\midrule
}
...
As mentioned above, it is helpful to check the predictions of the logical formalism against our intuition as speakers by “translating” the formulae into English or some other human language (i.e., replacing the variables \textit{p} and \textit{q} with sentences that express propositions). We noted at the beginning of \sectref{sec:4.2} that when we hear the sentence \textit{Either Joe is crazy or he is lying, and he is not crazy}, we seem to reach the conclusion \textit{Joe is lying} automatically and without effort. It takes a bit more effort to process a formula like $((p\vee q) \wedge (\neg p))$, but the table in \REF{ex:4.14} shows that the logical implication of this formula matches our intuition about the corresponding sentence.

Now consider the biconditional formula $(p\vee q) \leftrightarrow \neg((\neg p) \wedge (\neg q))$. Using the procedure outlined above, we can construct the truth table in \REF{ex:4.15}. First, columns 1 \& 2 are used to construct column 3, based on the truth table for \textit{$\vee$}. Next, columns 4 \& 5 are used to construct column 6, based on the truth table for \textit{$\wedge$}. Column 7 is calculated from column 6, and finally columns 3 \& 7 are used to construct column 8, based on the truth table for $\leftrightarrow$.
...
\lsptoprule
\tablehead{%
 p & q & p $\rightarrow$ q & (p $\rightarrow$ q) $\wedge$ p & ((p $\rightarrow$ q) $\wedge$ p) $\rightarrow$ q\\\midrule
}
...
Modus ponens defines one of the valid ways of deriving an inference from a conditional statement. It says that if we know that $p\rightarrow q$ is true, and in addition we know or assume that \textit{p} is true, it is valid to infer that \textit{q} is true. An illustration of this pattern of inference is presented as a \textsc{syllogism} in \REF{ex:4.17}.

\settowidth\jamwidth{(p→q)}
\ea \label{ex:4.17}
Premise 1: \textit{If John is Estonian, he will like this book.}   \jambox{($p\rightarrow q$)}
...
Another valid rule for deriving an inference from a conditional statement is \textsc{modus tollens} ‘method of rejecting/denying’, also called ‘denying the consequent’: $((p\rightarrow q) \wedge \neg q) \rightarrow \neg p$. This rule was illustrated in example (\ref{ex:4.4}a) above, repeated here as \REF{ex:4.18}. It says that if we know that $p\rightarrow q$ is true, and in addition we know or assume that \textit{q} is false, it is valid to infer that \textit{p} is also false.

\settowidth\jamwidth{(p→q)}
\ea \label{ex:4.18}
Premise 1: \textit{If dolphins are fish, they are cold-blooded.} \jambox{($p\rightarrow q$)}
Premise 2: \textit{Dolphins are not cold-blooded}.   \jambox{($\neg q$)}
\FelixHRule
Conclusion: \textit{Dolphins are not fish}.   \jambox{($\neg p$)}
\z


The tautology which we proved in \REF{ex:4.14} is known as the \textsc{disjunctive syllogism}: $((p\vee q) \wedge (\neg p)) \rightarrow q$. Another example which illustrates this pattern of inference is provided in \REF{ex:4.19}.


\ea \label{ex:4.19}
Premise 1: \textit{Dolphins are either fish or mammals}.  \jambox{($p\vee q$)}
Premise 2: \textit{Dolphins are not fish}.  \jambox{($\neg p$)}
\FelixHRule
Conclusion: \textit{Dolphins are mammals}.   \jambox{(q)}
\z


Finally, the tautology known as the \textsc{hypothetical syllogism} is given in \REF{ex:4.20}.


\ea \label{ex:4.20}
$((p\rightarrow q) \wedge (q\rightarrow r)) \rightarrow (p\rightarrow r)$\\
Premise 1: \textit{If Mickey is a rodent, he is a mammal}.  \jambox{($p\rightarrow q$)}
Premise 2: \textit{If Mickey is a mammal, he is warm-blooded}.  \jambox{($q\rightarrow r$)}
\FelixHRule
Conclusion: \textit{If Mickey is a rodent, he is warm-blooded}.  \jambox{($p\rightarrow r$)}
\z
...
In our logical notation we will write predicates in capital letters (to distinguish them from normal English words) and without inflectional morphology. We follow the common practice of using lower case initials to represent proper names. For predicates which require two arguments, the agent or experiencer is normally listed first. So the simple sentence \textit{John is hungry} would be translated into the logical metalanguage as $\text{HUNGRY}(j)$, while the sentence \textit{John loves Mary} would be translated $\text{LOVE}(j,m)$. Some additional examples are shown in \REF{ex:4.23}.


\eabox{\label{ex:4.23}
\begin{tabular}{@{} l @{~} >{\raggedright}p{.4\textwidth} l @{}}
a. & \textit{Henry VIII snores.} & $\text{SNORE}(h)$\\
b. & \textit{Socrates is a man.} & $\text{MAN}(s)$\\
c. & \textit{Napoleon is near Paris.} & $\text{NEAR}(n,p)$\\
d. & \textit{Abraham Lincoln admired Queen Victoria.} & $\text{ADMIRE}(a,v)$\\
e. & \textit{Jocasta is Oedipus’ mother}. & $\text{MOTHER\_OF}(j,o)$\\
f. & \textit{Abraham Lincoln was tall and homely.} & $\text{TALL}(a) \wedge \text{HOMELY}(a)$\\
g. & \textit{Abraham Lincoln was a tall man.}  & $\text{TALL}(a) \wedge \text{MAN}(a)$\\
h. & \textit{Joe is neither honest nor competent}. & $\neg (\text{HONEST}(j) \vee \text{COMPETENT}(j))$
\end{tabular}}
...
\settowidth\jamwidth{$\exists x[\text{LAWYER}(x) \wedge \text{HONEST}(x)]$}
\ea \label{ex:4.32}
\ea  \textit{All men are mortal.}  \jambox{$\forall x[\text{MAN}(x) \rightarrow \text{MORTAL}(x)]$}
\textit{Socrates is a man}.        \jambox{$\text{MAN}(s)$}
\FelixHRule
\textit{Therefore,} \textit{Socrates is mortal.}  \jambox{$\text{MORTAL}(s)$}
\bigskip

\ex   \textit{Arthur is a lawyer.}                \jambox{$\text{LAWYER}(a)$}
\textit{Arthur is honest}.                        \jambox{$\text{HONEST}(a)$}
\FelixHRule
\textit{Therefore,} \textit{some (= at least one)}  \jambox{$\exists x[\text{LAWYER}(x) \wedge \text{HONEST}(x)]$}
...
\item Modus tollens:  $[(p \rightarrow q) \wedge \neg q]  \rightarrow  \neg p$
\end{enumerate}
...
\item  Modus tollens:  $[(p \rightarrow  q) \wedge \neg q]  \rightarrow   \neg p$
\item  Hypothetical syllogism:  $[(p \rightarrow  q) \wedge (q \rightarrow  r)]  \rightarrow   (p \rightarrow  r)$
\item  Disjunctive syllogism:  $[(p \vee q) \wedge \neg p]  \rightarrow   q$
\end{enumerate}
...
Construct truth tables for these two sentences to show why they are logically equivalent. You may omit the adverbial clause (\textit{until the trial is finished}) from your table.  (\textit{Hint}: Let $p$ stand for \textit{I will read the newspaper} and $q$ stand for \textit{I will watch television}. Assume the following translation for sentence (a): $\neg (p \vee q)$. Construct a truth table for this proposition, and a second truth table for sentence (b). If the right-most column of the two tables is identical, that means that the two propositions must have the same truth value under any circumstances.)
