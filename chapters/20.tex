...
\paragraph*{A: (Patterned after \citet[225]{Kearns2000}.) Identify the most likely situation type (\textit{aktionsart}) for the following predicates (possible answers: \textsc{state, activity, achievement, accomplishment, semelfactive}):}
...
\paragraph*{A: (Adapted from \citet[147]{Saeed2009}.) Some English verbs are polysemous between a stative sense and a dynamic (eventive) sense. Show how the progressive aspect can be used to distinguish these two senses for each of the following five verbs: \textit{weigh, extend, surround, smell}, \textit{apply} (e.g. \textit{that law doesn’t apply} vs. \textit{apply for a job}).}~\\
