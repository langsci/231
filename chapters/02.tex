...
Let us begin by discussing the relationships between a speaker’s words, the situation in the world, and the thoughts or concepts associated with those words. These relationships are indicated in \figref{ex:2.2}, which is a version of a diagram that is sometimes referred to as the Semiotic Triangle.
...
\gll ahor-ita\\
     now-\textsc{dim}\\
\glt ‘soon, in a little while’ (Caribbean \ili{Spanish})

\ex \emph{Intensification}\\
\ea
\gll ahor-ita\\ 
     now-\textsc{dim}\\
\glt ‘immediately, right now’ (\ili{Latin} American \ili{Spanish})
...
\fittable{%
\begin{tabular}{lll}
a. &  \small cordates (=‘animals with hearts’)  & \small renates (= ‘animals with kidneys’) \footnote{The terms \textit{cordate} and \textit{renate} were coined by the philosopher  W.V.O. \citet{Quine1970}.} \\
