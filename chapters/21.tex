...
One way to represent the “location” of a situation in time is to define logical operators (e.g.~PAST and FUTURE) which will add tense information to a basic proposition. These tense operators can be defined as existential quantifiers over times, as suggested in \REF{ex:21.3}.\footnote{\citet{Prior1957,Prior1967}.} This definition says that $\text{PAST}(p)$ will be true at the time of speaking just in case there was some time prior to the time of speaking when $p$ was true; and similarly for $\text{FUTURE}(p)$. (The letter \textit{t} stands for ‘time’, and “{$<$}” in this context means ‘prior to’. TU represents the time of speaking; this is typically the time for which the truth value of a statement is evaluated.)


\ea \label{ex:21.3}
$\text{PAST}(p)$ is true at TU  iff  $\exists t [t < \text{TU} \wedge (p \text{ is true at time } t)]$\\
$\text{FUTURE}(p)$ is true at TU  iff  $\exists t [\text{TU} < t \wedge (p \text{ is true at time } t)]$
\z
...
\glt ‘Having delayed a little while, he orders them to give the signal.’\footnote{Example \REF{ex:21.21} comes from Sallust, Catilina 59; cited in \citet[§496]{AllenGreenough1903}. The past participle in \ili{Latin}, as in English, normally has a passive meaning; but the verb meaning ‘delay’ in \ili{Latin} is a \textsc{deponent} verb, meaning that passive morphology does not create a passive meaning.}\\
...
\exbox{
\begin{tabularx}{.9\textwidth}{cccY}
TU &  [ TT: Mon. eve. ] &   PT &  “past in the future” (\ref{ex:21.24}a)\\
\cline{1-3}
...
\exbox{
\begin{tabularx}{.9\textwidth}{cccY}
PT &  [ TT: 1922 ] &   TU &  “future in the past” (\ref{ex:21.24}b)\\
\cline{1-3}
...
However, a number of languages have verbal affixes which distinguish more than one degree of past and/or future time reference, e.g. ‘immediate past’ vs. ‘near past’ vs. ‘distant past’. Such systems are especially well-known among the  {Bantu} languages. Example \REF{ex:21.37} presents a paradigm from the {Bantu} language \ili{ChiBemba}, which has (in addition to the present tense, not shown here) a symmetric set of four past and four future time markers.\largerpage
...
\begin{tabularx}{.85\textwidth}{cccY}
...
\begin{tabularx}{.8\textwidth}{cccY}
