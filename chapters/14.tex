...
\z \z
\footnotetext{Recall from \chapref{sec:9} that numerals seem to allow two different interpretations. In light of that discussion, this sentence could mean either $|S \cap B| = 4$ or $|S \cap B|\geq 4$ depending on context. For the purposes of this chapter we will ignore the ‘at least’ reading.}

Notice that we have distinguished plural vs. singular uses of \textit{some} by stating that plural \textit{some} (ex. \ref{ex:14.5}c) indicates an intersection with cardinality of two or more. The interpretation suggested in \REF{ex:14.5h} indicates that the meaning of \textit{both} includes the subset relation and the assertion that the cardinality of the first set equals two. This amounts to saying that \textit{both} means ‘all two of them’. Strictly speaking, it might be more accurate to treat the information about cardinality as a presupposition, because that part of the meaning is preserved in questions (\textit{Are} \textit{both students brilliant?}), conditionals (\textit{If} \textit{both students are brilliant, then …}), etc. However, we will not pursue that issue here.
...
\ea \label{ex:14.10}
\ea  {\textit{All men snore}}.  $\forall x [\text{MAN}(x) \rightarrow \text{SNORE}(x)]$\\
\ex  {\textit{No women snore}}.  $\neg\exists x[\text{WOMAN}(x) \wedge \text{SNORE}(x)]$\\
    \ex  \textit{Some man snores}.  $\exists x[\text{MAN}(x) \wedge \text{SNORE}(x)]$
     \z
\z


Now we are in a position to understand why these forms work as translations of the English quantifier words. The use of material implication ($\rightarrow$) in (\ref{ex:14.10}a) follows from the definition of the subset relation which we presented in \chapref{sec:13}, repeated here in (\ref{ex:14.11}a). The use of logical $\wedge$ ‘and’ in (\ref{ex:14.10}b--c) follows from the definition of set intersection presented in \chapref{sec:13}, repeated here in (\ref{ex:14.11}b).
...
($\llbracket \text{MAN}\rrbracket \cap \llbracket \text{SNORE}\rrbracket \neq ∅$) $\leftrightarrow  \exists x[(x \in \llbracket \text{MAN} \rrbracket) \wedge (x \in \llbracket \text{SNORE} \rrbracket)]$
\z \z
...
\ex \textit{Few strict Baptists drink or smoke}.\\
  {}[\textit{few} $x$: $\text{BAPTIST}(x) \wedge \text{STRICT}(x)$] $\text{DRINK}(x) \vee \text{SMOKE}(x)$\footnote{This logical rendering of the sentence meaning ignores the fact that \textit{strict} is actually a non-intersective adjective. See \chapref{sec:15} for discussion.}
   \z
\z
...
\ea \label{ex:14.15}
\ea \relax [\textit{all} $x$: $P(x)$] $Q(x)  \leftrightarrow   \llbracket P\rrbracket  \subseteq \llbracket Q \rrbracket$ \\
\ex{} [\textit{no} $x$: $P(x)$] $Q(x)  \leftrightarrow   \llbracket P \rrbracket \cap \llbracket Q \rrbracket = ∅$ \\
\ex{} [\textit{four} $x$: $ P(x)$] $Q(x) \leftrightarrow   | \llbracket P \rrbracket \cap \llbracket Q \rrbracket| =  4$\\
\ex{} [\textit{most} $x$: $ P(x)$] $Q(x)  \leftrightarrow  | \llbracket P \rrbracket \cap \llbracket Q \rrbracket | >  \text{½} |\llbracket P \rrbracket|$
     \z
\z
...
$|\llbracket \text{STUDENT} \rrbracket \cap \llbracket \text{BRILLIANT} \rrbracket | > \text{½} | \llbracket \text{STUDENT} \rrbracket|$
\ex \textit{No women snore}.\\
{}[\textit{no} $x$: $ \text{WOMAN}(x)$] $\text{SNORE}(x)$\\
$\llbracket \text{WOMAN} \rrbracket \cap \llbracket \text{SNORE} \rrbracket = ∅$\\
...
\ea \relax [\textit{many} $x$: $\text{BOOK}(x) \wedge \text{VALUABLE}(x)$] $\neg \text{FIND}(\text{speaker},x)$\\
...
\ea  {[\textit{every} $x$: $\text{STUDENT}(x)$]} ([\textit{two} $y$:  $\text{LANGUAGE}(y)$] $\text{KNOW}(x, y)$)\\
\ex  {[\textit{two} $y$: $\text{LANGUAGE}(y)$]} ([\textit{every} $x$: $\text{STUDENT}(x)$] $\text{KNOW}(x, y)$)
   \z
\z
...
\section{Conclusion}\label{sec:14.6}\largerpage
...
These concepts will be important in later chapters, especially in \chapref{sec:16} where we discuss modality. As discussed in that chapter, a very influential analysis of modality is based on the claim that modal expressions like \textit{may}, \textit{must}, \textit{could}, etc. are really a special type of quantifier.\largerpage
...
\shortmodelanswer{Model answer}{[\textit{every} $x$: $\text{ROMAN}(x)$] $\text{PATRIOTIC}(x)$\\
...
\shortmodelanswer{Model answer}{$\forall x[\text{RIDDLE}(x) \rightarrow \text{ANSWER}(s,x)]$}
...
\shortmodelanswer{Model answer}{[Every $x$: $\text{THING}(x) \wedge \text{COOK}(s,x)$] $\text{EAT}(a,x)$}
...
\shortmodelanswer{Model answer}{$|\llbracket \text{SENATOR} \rrbracket \cap \llbracket \text{GUILTY} \rrbracket | > 20$}
