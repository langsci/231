...
Similarly, the analysis predicts that both (\ref{ex:16.20}a) and (\ref{ex:16.20}b) should be true, because the antecedent will be false in all possible worlds consistent with what the law requires. (Recall from \chapref{sec:4} that \textit{p→q} is always considered to be true when \textit{p} is false.)
...
Epistemic: [all w: (w is consistent with the available evidence) $\wedge$ (the normal course of events is followed as closely as possible in w)] TRUST(a,h) in w\\
Deontic: [all w: (the relevant circumstances of the actual world are also true in w) $\wedge$ (the relevant authority’s requirements are satisfied as completely as possible in w)] TRUST(a,h) in w} \\
